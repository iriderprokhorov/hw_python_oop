...
class SportsWalking(Training):
    ...
    def get_spent_calories(self) -> float:
        ...
        mean_speed = self.get_mean_speed()
        duration_in_min = self.duration * 60
        spent_calories = ((coeff_calorie_3 * self.weight +
...
def read_package(workout_type: str, data: list) -> Training:
    ...
    if workout_type == 'SWM':
        return dict_sports['SWM'](action=data[0], duration=data[1],
                                  weight=data[2], length_pool=data[3],
                                  count_pool=data[4])
    elif workout_type == 'RUN':
        return dict_sports['RUN'](action=data[0], duration=data[1],
                                  weight=data[2])
    elif workout_type == 'WLK':
        return dict_sports['WLK'](action=data[0], duration=data[1],
